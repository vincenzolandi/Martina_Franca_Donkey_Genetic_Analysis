...
pedigree_cleaned <- pedigree_data[, c("Progeny", "Sire", "Dam", "year")] %>%
  rename(
    animal = Progeny,
    sire = Sire,
    dam = Dam,
...
# Add a new column for province based on the CODICE ASL column "CODASL" (in our case this is the farm national ID that contain provice indication and a serial for aeach farm
...
p1 <- ggplot(average_inbreeding_per_year_filtered, aes(x = year)) +
